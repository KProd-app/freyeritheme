...
// === DISCOUNT MODAL (robust) ===
document.addEventListener("DOMContentLoaded", () => {
  const modal = document.getElementById("discountModal");
  if (!modal) return;

  // 1) Atidarymas iš Rich text mygtuko (Link = #discount)
  document.querySelectorAll('a[href="#discount"], button[href="#discount"]').forEach(el => {
    el.addEventListener("click", (e) => { e.preventDefault(); open(); });
  });

  const closeBtn   = modal.querySelector(".discount-modal__close");
  const form       = document.getElementById("discountForm");
  const emailInput = document.getElementById("discountEmail");
  const errorMsg   = modal.querySelector(".discount-modal__error");
  const successBox = modal.querySelector(".discount-modal__success");
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    // RODYK KODĄ IŠKART (optimistinis UI)
    form.classList.add("hidden");
    successBox.classList.remove("hidden");

    // 3) Fone bandome užregistruoti prenumeratą Shopify'e
    try {
      const fd = new FormData();
      fd.append("form_type", "customer");
      fd.append("utf8", "✓");
      fd.append("contact[email]", email);
      fd.append("contact[tags]", "newsletter, discount-10");
      ...
      // jei reikia – čia galima uždėti žalią pranešimą
      console.log("Newsletter saved in Shopify");
    } catch (err) {
      console.warn("Newsletter save failed (code still shown):", err);
    }
  });
});
